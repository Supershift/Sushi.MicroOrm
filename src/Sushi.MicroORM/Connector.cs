...
Please map identity primary key column using Map.Id(). Otherwise use Insert or Update explicitly.");
        ...
        /// </summary>        
        /// <returns></returns>
        private async Task<SqlStatementResult<TResult>> ExecuteSqlStatementAsync<TResult>(SqlStatement statement, ConnectionString? connectionString, int? commandTimeout,
            bool isReadOnly, CancellationToken cancellationToken)
        {
            ...
            if (isReadOnly && connectionString.ReadOnly != null)
                connectionStringValue = connectionString.ReadOnly;
            else
                connectionStringValue = connectionString.Primary;

            var result = await _sqlExecuter.ExecuteAsync<T, TResult>(statement, connectionStringValue, commandTimeout, _map, cancellationToken);
            return result;
        }
